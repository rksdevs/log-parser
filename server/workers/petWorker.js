...
async function processPetLog(filePath, logId) {
  ...
  const { petOwners, allGuids } = resolvePetRelationsFromLogs(rawLogLines);
  ...
  fs.writeFileSync(
    outputPath,
    JSON.stringify({ logId, output, petOwners, allGuids }, null, 2)
  );
  console.log(`✅ Pet processing complete. Output written to ${outputPath}`);
}

processPetLog("../server/logs/json/Sample-togc-10-inno.txt", 151);
