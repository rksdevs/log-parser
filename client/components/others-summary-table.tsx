...
export default function OthersSummaryTable() {
  const { logSummary } = useNavigation();
  const [allOthersData, setAllOthersData] = useState<PlayerStats[]>([]);
  ...
  useEffect(() => {
    if (!logSummary?.otherStats) return;

    const others = logSummary.otherStats;

    const maxPlayerDamage = Math.max(...others.map((p) => p.totalDamage));
    const totalPlayerDamage = others.reduce((sum, p) => sum + p.totalDamage, 0);

    const playersWithGraphData = others.map((p) => ({
      ...p,
    ...
    setAllOthersData(playersWithGraphData);
  }, [logSummary]);
  ...
  const columns = [
    ...
    columnHelper.accessor("totalDamage", {
      ...
      cell: ({ row }) => {
        ...
        const max = Math.max(...allOthersData.map((p) => p.totalDamage));
        const ratio = max ? current / max : 0;
        const total = allOthersData.reduce((sum, p) => sum + p.totalDamage, 0);
  ...
  const table = useReactTable({
    ...
    pageCount: Math.ceil(allOthersData.length / pagination.pageSize),
    manualPagination: false,
    onPaginationChange: setPagination,
  });

  if (!allOthersData.length) return <div>Loading...</div>;
