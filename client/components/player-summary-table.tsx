...
export default function PlayerSummaryTable() {
  ...
  useEffect(() => {
    if (!logSummary?.playerStats) return;

    const players = logSummary.playerStats;

    const maxPlayerDamage = Math.max(...players.map((p) => p.totalDamage));
    const totalPlayerDamage = players.reduce(
      (sum, p) => sum + p.totalDamage,
      0
    );

    const playersWithGraphData = players.map((p) => ({
      ...p,
    ...
    setAllPlayersData(playersWithGraphData);
  }, [logSummary]);
  ...
  const columns = [
    ...
    columnHelper.accessor("totalDamage", {
      ...
      cell: ({ row }) => {
        ...
        const max = Math.max(...allPlayersData.map((p) => p.totalDamage));
        const ratio = max ? current / max : 0;
        const total = allPlayersData.reduce((sum, p) => sum + p.totalDamage, 0);
  ...
  const table = useReactTable({
    ...
    pageCount: Math.ceil(allPlayersData.length / pagination.pageSize),
    manualPagination: false,
    onPaginationChange: setPagination,
  });

  if (!allPlayersData.length) return <div>Loading...</div>;
