...
interface BasicSummaryTableProps {
  data: PlayerStats[];
}
...
export default function BasicSummaryTable({ data }: BasicSummaryTableProps) {
  ...
  const columns = [
    ...
    columnHelper.accessor("totalDamage", {
      ...
      cell: ({ row }) => {
        ...
        const max = Math.max(...data.map((p) => p.totalDamage));
        const ratio = max ? current / max : 0;
        const total = data.reduce((sum, p) => sum + p.totalDamage, 0);
