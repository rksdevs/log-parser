...
export default function HealingTable({ data }: DamageTableProps) {
  ...
  const columns = [
    columnHelper.accessor("playerName", {
      ...
      cell: ({ row }) => (
        <Link
          ...
          href={
            startTime
              ? `/${logId}/${startTime}/${row.original.playerName}?class=${row.original.class}`
              : `/${logId}/player/${row.original.playerName}?class=${row.original.class}`
          }
        >
    ...
    columnHelper.accessor("totalHealing", {
      ...
      enableSorting: true,
      cell: ({ row }) => {
        const current = row.original.totalHealing;
        const max = Math.max(...data.map((p) => p.totalHealing));
        const ratio = max ? current / max : 0;
        const total = data.reduce((sum, p) => sum + p.totalHealing, 0);
  ...
  if (!data.length) return <div>No data available to show</div>;
