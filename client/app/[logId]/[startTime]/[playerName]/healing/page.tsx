...
export default function PlayerSpellsPageHealing() {
  const { playerSpells, petSpells, loading, playerClass } =
    usePlayerSpellsFromAttempt();
  const { playerName } = useParams();
  const [sorting, setSorting] = useState<SortingState>([
    { id: "healingForTable", desc: true },
  ]);
  const [pagination, setPagination] = useState({
    pageIndex: 0,
    pageSize: 10, // Show 10 rows per page
  });
  const [playerHealingSpells, setPlayerHealingSpells] = useState<SpellData[]>(
    []
  );
  const [petHealingSpells, setPetHealingSpells] = useState<SpellData[]>([]);
  const [healingDataAvailable, setHealingDataAvailable] =
    useState<boolean>(false);

  useEffect(() => {
    if (playerSpells) {
      const healingSpells = playerSpells.filter(
        (spell) => (spell.healing?.effective ?? 0) > 0
      );
      if (healingSpells.length) {
        setPlayerHealingSpells(healingSpells);
        setHealingDataAvailable(true);
      }
    } else {
      return;
    }
  ...
  useEffect(() => {
    if (petSpells) {
      const pethealingSpells = petSpells.filter(
        (spell) => (spell.healing?.effective ?? 0) > 0
      );
      if (pethealingSpells.length) {
        setPetHealingSpells(pethealingSpells);
        setHealingDataAvailable(true);
      }
    } else return;
  }, [petSpells]);
  ...
  const columns: ColumnDef<SpellData, any>[] = [
    ...
    // }),
    columnHelper.accessor("healingForTable", {
      id: "healingForTable",
      header: ({ column }) => (
        <div
        ...
        >
          Healing <ArrowUpDown className="w-4 h-4" />
        </div>
      ),
      enableSorting: true,
      sortingFn: (a, b) => {
        return (
          (a.original.healingForTable ?? 0) - (b.original.healingForTable ?? 0)
        );
      },
      cell: ({ row }) => {
        const current = row.original.healingForTable ?? 0;
        const max = Math.max(
          ...playerHealingSpells.map((s) => s.healingForTable ?? 0)
        );
        const ratio = max ? current / max : 0;
        const total = playerHealingSpells.reduce(
          (sum, s) => sum + (s.healingForTable ?? 0),
          0
        );
    ...
    columnHelper.accessor((row) => row.healing?.hps ?? 0, {
      id: "hps",
      header: "HPS",
    ...
    columnHelper.accessor((row) => row.healing?.avgHit ?? 0, {
      id: "avgHit",
      header: "Avg Hit",
      cell: (info) => info.getValue().toFixed(1),
      meta: { className: "text-center w-[100px]" },
    }),
    columnHelper.accessor((row) => row.healing?.crits ?? 0, {
      id: "crits",
      header: "Crits",
      cell: (info) => info.getValue().toLocaleString(),
      meta: { className: "text-center w-[80px]" },
    }),
    columnHelper.accessor((row) => row.healing?.overheal ?? 0, {
      id: "overheal",
      header: "Overheal",
      cell: (info) => info.getValue(),
      meta: { className: "text-center w-[80px]" },
    }),
    // columnHelper.accessor((row) => row.healing?.overkill ?? 0, {
    //   id: "overkill",
    //   header: "Overkill",
    //   cell: (info) => info.getValue().toLocaleString(),
    //   meta: { className: "text-center w-[100px]" },
    // }),
    // columnHelper.accessor((row) => row.healing?.["uptime %"] ?? 0, {
    //   id: "uptime",
    //   header: "Uptime %",
    //   cell: (info) => info.getValue().toFixed(1),
  ...
  const playerTable = useReactTable({
    ...
    pageCount: Math.ceil(playerHealingSpells.length / pagination.pageSize),
  ...
  const petTable = useReactTable({
    ...
    state: { sorting },
    onSortingChange: setSorting,
    pageCount: Math.ceil(petHealingSpells.length / pagination.pageSize),
    manualPagination: false,
    onPaginationChange: setPagination,
    getPaginationRowModel: getPaginationRowModel(),
  });
  ...
  if (!healingDataAvailable) return <div> Oops... The guy doesnt heal!</div>;
  ...
  return (
    <div className="flex w-full flex-col justify-start gap-6 p-6 max-w-full">
      ...
      {petHealingSpells.length ? (
        <Card>
          <CardHeader>
            <CardTitle>{petHealingSpells[0]?.petName}</CardTitle>
            <CardDescription>
              {petHealingSpells[0]?.petName} Spell breakdown
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="rounded-lg border overflow-hidden">
